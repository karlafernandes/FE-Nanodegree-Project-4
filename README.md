...
5. Used PageSpeed Insights to check the score and used its tips to get better results:
	* Exchanged de CSS to inline
	* Unblocked some JS using async
	* Got great results to both Mobile and Desktop running at 94/100 Speed
...
2. Used PageSpeed Insights tips to get better results
	* Exchanged CSS font-size to get better view in mobile
	* Inlines the CSS inline to not let it block the process
	* Inspect my CSS and validated it using http://jigsaw.w3.org
	* Got a bit better score score: Mobile 79 / Desktop 96 from 100 Speed
...
5. Found the critical points in the slider and scrolling both in main.js
	* The scrolling was affected by the updatePositions function which instead to create all elements every time I would school I took the var items to out of the function and added all elements in a list running it just once to populate the list with all the pizzas
	* Worked on the function changeSliderLabel To NOT USE (determineDx / changePizzaSizes) var and functions anymore avoiding the big amount of work with painting and rendering that were very expensive processes using more than 16ms
